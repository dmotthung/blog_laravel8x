...
@section('content')
    <form action="{{ route('categories.store') }}" method="POST" class="form-create-category">
        @csrf
        <!-- title -->
        ...
        <div class="form-group">
            ...
            <textarea id="input_category_description" name="description" class="form-control @error('description') is-invalid @enderror" rows="5" placeholder="{{ trans('categories.form_control.textarea.description.placeholder') }}">{{ old('description') }}</textarea>
        ...
        <div class="float-right">
            ...
            <button type="submit" class="btn btn-primary px-4">{{ trans('categories.button.create.value') }}</button>
        </div>
    </form>
@endsection
